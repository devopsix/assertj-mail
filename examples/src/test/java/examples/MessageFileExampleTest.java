...
public class MessageFileExampleTest {
  ...
  public void loadMessage() throws IOException, MessagingException {
    try (InputStream messageStream = getClass().getClassLoader()
  ...
  public void messageShouldHaveValidDkimSignature() {
    ...
    final Map<String, String> gmailPublicKey = singletonMap("20161025._domainkey.gmail.com",
        "k=rsa; p=MIIBIjANBgkqhkiG9w0BAQEFAAOCAQ8AMIIBCgKCAQEAviPGBk4ZB64UfSqWyAicdR7lodhytae+E"
            + "YRQVtKDhM+1mXjEqRtP/pDT3sBhazkmA48n2k5NJUyMEoO8nc2r6sUA+/Dom5jRBZp6qDKJOwjJ5R/OpHa"
            + "mlRG+YRJQqRtqEgSiJWG7h7efGYWmh4URhFM9k9+rmG/CwCgwx7Et+c8OMlngaLl04/bPmfpjdEyLWyNim"
            + "k761CX6KymzYiRDNz1MOJOJ7OzFaS4PFbVLn0m5mf0HVNtBpPwWuCNvaFVflUYxEyblbB6h/oWOPGbzoSg"
            + "tRA47SHV53SwZjIsVpbq4LxUW9IxAEwYzGcSgZ4n5Q8X8TndowsDUzoccPFGhdwIDAQAB");
  ...
  public void messageShouldHaveAlternativeTextAndHtmlContent() {
    ...
    MailAssertions.assertThat(message).multipartContent().parts()
        .anySatisfy(part -> {
          MailAssertions.assertThat(part).multipartContent().contentType()
              .startsWith("multipart/alternative;");
          MailAssertions.assertThat(part).multipartContent().parts().satisfiesExactly(
              innerPart -> {
                MailAssertions.assertThat(innerPart).headerValue("Content-Type")
                    .startsWith("text/plain;");
                MailAssertions.assertThat(innerPart).textContent()
    ...
    MailAssertions.assertThat(message).multipartContents().anySatisfy(
        multipart -> {
          MailAssertions.assertThat(multipart).contentType()
              .startsWith("multipart/alternative;");
          MailAssertions.assertThat(multipart).parts().satisfiesExactly(
              part -> {
                MailAssertions.assertThat(part).headerValue("Content-Type")
                    .startsWith("text/plain;");
  ...
  public void messageShouldHaveImageAttachment() {
    ...
    MailAssertions.assertThat(message).multipartContent().parts()
        .anySatisfy(part -> {
          ...
          MailAssertions.assertThat(part).headerValue("Content-Disposition")
              .contains("lena.jpg");
